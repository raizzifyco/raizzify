...
const Hero = () => {
  ...
  if (section === "partner") {
    firstMarq[0].text = "Manage Your Shop";
  }
      ...
      <div className="flex flex-col items-center  w-full gap-y-8">
        <div className="font-bold lg:text-[56px] whitespace-nowrap sm:text-[38px] xs:text-[30px] text-[26px] md:px-0 px-6 md:w-3/4 w-full text-center md:leading-relaxed">
          ...
          <p className="text-center sm:block flex items-center flex-col tracking-tight">
            for
            <span className="bg-primary px-2 rounded-md text-white ml-4 leading-tight sm:inline-block block w-fit">
              {section === "salon" && "Salon Owners"}
              {section === "partner" && "Partners"}
            </span>
          </p>
        </div>
        <p className="text-secondaryTextColor lg:px-14 md:px-8 px-4 text-center lg:text-xl md:text-lg text-sm font-normal">
          {section === "salon" &&
            "Digitalize your salon, Maximize Revenue Insights, My Customers, My Bookings, My Team, Discounts"}
          {section === "partner" &&
            "Digitalize your shop, Maximize Revenue Insights, My Customers, My Bookings, My Team, Discounts"}
        </p>
        <div className="sm:flex hidden items-center gap-x-6">
          <Link href={`https://business.raizzify.com/`} target="_blank">
            <Button
              ...
              variant={"default"}
              className="w-[200px] py-6 font-semibold"
            >
              Get Started
            </Button>
          </Link>
      ...
      <div className="mt-10 relative w-full">
          ...
          <div className="marquee">
            <div className="marquee-content flex gap-x-4 items-center">
              {firstMarq.map((item, i) => (
                <div
                  key={i}
                  className="bg-white p-4 border border-[#00A210] rounded-md shadow-md"
                >
                  <Image
                    src={item.imageUrl}
        ...
        <div className="sm:hidden flex flex-col items-center gap-y-6 mt-6">
          <Button
            onClick={handleGetStarted}
            variant={"default"}
            className="sm:w-[200px] w-[300px] py-6"
          >
            Get Started
          </Button>
