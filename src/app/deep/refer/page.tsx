...
const DeepLinkHandler: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (ref) {
      const deferredLink = `raizz://www.raizzify.com/deep/refer?referrer=${uniqueId}`;

      const processDeepLink = async () => {
        await sendUniqueIdToBackend(uniqueId, `${deferredLink}&ref=${ref}`);
...
const generateUniqueId = () => {
  return `refer-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
};
